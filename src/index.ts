...
export default {
  async fetch(request: Request, env: any, ctx: ExecutionContext) {
    ...
    if (url.pathname === "/auth/verify") {
      ...
      try {
        ...
        if (Date.now() - decoded.stamp > fifteenMinutesInMs) {
          ...
          return new Response(`
            <html>
              <body style="font-family:sans-serif; background:#0b1120; color:#ef4444; text-align:center; padding-top:100px;">
                <h2>✕ Verification Window Expired</h2>
                <p style="color:#94a3b8; font-size:14px;">The 15-minute token lifecycle has concluded. Your pending entry was removed. Please re-register.</p>
              </body>
            </html>
    ...
    if (url.pathname === "/auth/register" && request.method === "POST") {
      ...
      if (!verifyIEC62443PasswordStrength(password)) return Response.json({ error: "Password rejection." }, { status: 400, headers: corsHeaders });
    ...
    if (url.pathname === "/auth/login" && request.method === "POST") {
      ...
      if (user.email_verified === 0) {
        return Response.json({ error: "Access Denied. Validate your account via the link sent to your email id first." }, { status: 403, headers: corsHeaders });
      }
      if (user.approved === 0) {
        return Response.json({ error: "Email verified! Account access is now pending Administrator authorization." }, { status: 403, headers: corsHeaders });
      }
